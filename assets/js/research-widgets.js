 ...
 *
 * Sections:
 *   1. #research-metrics     — h-index, citations, document count
 *   2. #publications-by-year — bar chart (pure CSS, no library)
 *   3. #fingerprint          — keyword cloud with percentage bars
 *   4. #sdg-section          — ODS/SDG icon badges
 *   5. #publications-list    — searchable/filterable publication list
 *
 * Usage:
...
(function () {
  ...
  // ── SDG icon URLs (official UN assets via jsdelivr CDN) ──────────────────
  const SDG_COLORS = {
  ...
  function renderPublications(container, publications, generated) {
    ...
    // controls
    const searchInput = el("input", { type: "text", placeholder: "Search publications…" });
